...
where $n$ is the sample size, $\epsilon_{i}$ is unit $i$'s residual, $X_{i}$ is unit $i$'s value in the $X$ variable, and $\overline{X}$ is the mean of $X$. This expression is easier to digest if you recognize some of its component parts. The numerator $\frac{1}{n}\sum_{i=1}^{n}\epsilon_{i}^2$ is the average of squared residuals. The denominator $\sum_{i=1}^{n}(X_{i}-\overline{X})^2$ represents the total variation in $X$ about its mean.
